...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const abVect = x1 * x2 + y1 * y2;
  const ab = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(abVect / ab);
}
...
function toNumber(value, def) {
  ...
  if (typeof numValue === 'number' && numValue === +numValue) return numValue;
